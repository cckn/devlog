...
Alarm and Event Services는 BACnet 장치에서 알람을 포함한 상태 변화를 처리합니다. 센서 판독 값이 정상 범위를 벗어났거나 정상 작동으로 돌아가는("이벤트") 문제 또는 오류 조건을 나타낼 수 있는 경보 및 이벤트 변경 또는 "Change Of Value" 또는 COV라는 이전 보고서 이후 일부 증가분의 판독값 변경입니다.
...
서비스 이름에서 **"Atomic"**이라는 단어는 한 번에 하나의 읽기 또는 쓰기 작업만 수행할 수 있음을 의미합니다. 액세스 중에 파일에 액세스하려는 다른 시도는 실패하거나 보류됩니다.
...
장치 통신 제어 및 다시 초기화합니다. 장치 서비스는 원격으로 호출될 수 있는 진단 도구를 인간 운영자에게 제공하기 위한 것입니다. BACnet 장치는 장치 통신 제어 및 다시 초기화를 제외한 모든 BACnet 메시지를 무시하도록 주문할 수 있습니다.
